...
export default function NovelReader({documentId,title,markdown,preview=false,onBack,onShare}:Props){
 const{settings,setSettings,ready}=useReaderSettings();const[toc,setToc]=useState(false);const[drawer,setDrawer]=useState(false);const[drawerDrag,setDrawerDrag]=useState(0);const[progress,setProgress]=useState(0);const[target,setTarget]=useState<string|null>(null);const pull=useRef<{y:number;closing:boolean}|null>(null);const headings=useMemo(()=>extractHeadings(markdown),[markdown]);
 ...
 const updateProgress=useCallback((n:number)=>setProgress(Math.max(0,Math.min(1,n))),[]);const widths={narrow:"640px",default:"720px",wide:"800px"};
 ...
 const pullDown=(e:React.PointerEvent)=>{const handle=(e.target as HTMLElement).closest(".drawer-handle");if(drawer&&!handle)return;if(!drawer&&e.clientY>54)return;pull.current={y:e.clientY,closing:drawer};e.currentTarget.setPointerCapture(e.pointerId)};const pullMove=(e:React.PointerEvent)=>{if(!pull.current)return;const dy=e.clientY-pull.current.y;setDrawerDrag(pull.current.closing?Math.min(0,Math.max(-320,dy)):Math.min(420,Math.max(0,dy)))};const pullEnd=()=>{if(pull.current?.closing&&drawerDrag<-70)setDrawer(false);else if(!pull.current?.closing&&drawerDrag>70)setDrawer(true);setDrawerDrag(0);pull.current=null};
 return <main className={`novel-reader theme-${settings.theme} font-${settings.fontFamily} mode-${settings.mode} ${preview?"is-preview":""}`} style={{"--reader-font-size":`${settings.fontSize}px`,"--reader-line-height":settings.lineHeight,"--reader-max-width":widths[settings.width]} as React.CSSProperties} onPointerDown={pullDown} onPointerMove={pullMove} onPointerUp={pullEnd} onPointerCancel={pullEnd}>
  <div className="reader-status"><span className="status-grip">⌄</span><strong>{title}</strong><span>{Math.round(progress*100)}%</span></div>
  <div className={`quick-drawer ${drawer?"is-open":""}`} style={drawerDrag?(drawer?{transform:`translateY(${drawerDrag}px)`}:{transform:`translateY(calc(-100% + ${drawerDrag}px))`}):undefined}>
   <button className="drawer-handle" onClick={()=>setDrawer(false)} aria-label="설정 닫기"><span/></button><div className="brightness-row"><span>화면 밝기</span><button className="sun-moon" onClick={()=>setSettings(s=>({...s,theme:s.theme==="light"?"dark":"light"}))} aria-label="밝기 테마 전환"><i className="pixel-sun">☀</i><i className="pixel-moon">☾</i><b className={settings.theme==="dark"?"at-dark":""}/></button></div><ReaderSettings value={settings} onChange={setSettings} onClose={()=>setDrawer(false)} embedded/>
  </div>
  <div className="reader-body">{settings.mode==="scroll"?<ScrollReader markdown={markdown} progress={progress} onProgress={updateProgress} restore={progress}/>:<PaginatedReader markdown={markdown} progress={progress} onProgress={updateProgress} onUiToggle={()=>setDrawer(v=>!v)} goToId={target}/>}</div>
