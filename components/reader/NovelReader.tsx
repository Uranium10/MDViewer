...
export default function NovelReader({documentId,title,markdown,preview=false,onBack,onCreateShare,onPageChange}:Props){
  ...
  const downloadDocx=async()=>{flash("DOCX 파일을 만드는 중입니다.");try{const exporter=await import("@/lib/docx-export");await exporter.downloadDocx(markdown,title);flash("DOCX 저장을 시작했습니다.")}catch{flash("DOCX 파일을 만들지 못했습니다.")}};
  ...
  const confirmBack=()=>{setLeaveConfirm(false);if(onBack)return onBack();location.assign("/")};
