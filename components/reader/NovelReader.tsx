...
type Props={documentId:string;title:string;markdown:string;preview?:boolean;onBack?:()=>void;onCreateShare?:()=>Promise<string|undefined>};
type Popover="share"|"save"|null;
type StoredSession={progress?:unknown;bookmarks?:unknown};

export default function NovelReader({documentId,title,markdown,preview=false,onBack,onCreateShare}:Props){
  const{settings,setSettings,ready}=useReaderSettings();
  const[toc,setToc]=useState(false),[drawer,setDrawer]=useState(false),[guide,setGuide]=useState(false),[popover,setPopover]=useState<Popover>(null),[printing,setPrinting]=useState(false);
  ...
  const openGuide=useCallback(()=>{if(guide)return;setDrawer(false);setToc(false);setPopover(null);history.pushState({...history.state,mdBooksGuide:true},"",location.href);setGuide(true)},[guide]);
  const closeGuide=useCallback(()=>{if(history.state?.mdBooksGuide)history.back();else setGuide(false)},[]);
  ...
  const pullDown=(event:React.PointerEvent)=>{if((event.target as HTMLElement).closest("button,input"))return;const handle=(event.target as HTMLElement).closest(".drawer-handle");if(drawer&&!handle)return;const top=readerNode.current?.getBoundingClientRect().top??0;if(!drawer&&event.clientY-top>54)return;pull.current={y:event.clientY,closing:drawer};drawerNode.current?.classList.add("is-dragging");event.currentTarget.setPointerCapture(event.pointerId)};
  ...
  return <main ref={readerNode} className={`novel-reader theme-${settings.theme} font-${settings.fontFamily} mode-${settings.mode} ${preview?"is-preview":""}`} style={{"--reader-font-size":`${settings.fontSize}px`,"--reader-line-height":settings.lineHeight,"--reader-max-width":widths[settings.width]} as React.CSSProperties} onPointerDown={pullDown} onPointerMove={pullMove} onPointerUp={pullEnd} onPointerCancel={pullEnd}>
    <div className="reader-status"><span className="status-grip">⌄</span><strong>{guide?"MD북스 도움말":title}</strong><button className="reader-help" onClick={openGuide} aria-label="MD북스 도움말 열기" aria-expanded={guide}><i className="pixel-icon icon-help" aria-hidden="true">?</i></button></div>
    ...
    <div className="reader-body">{settings.mode==="scroll"?<ScrollReader markdown={markdown} progress={progress} onProgress={updateProgress} restore={progress}/>:<PaginatedReader key={`${settings.fontSize}:${settings.lineHeight}:${settings.fontFamily}:${settings.width}`} markdown={markdown} progress={progress} bookmarks={bookmarks} onBookmarksChange={updateBookmarks} onProgress={updateProgress} onUiToggle={toggleDrawer} goToId={target}/>}</div>
    ...
    {toc&&<ReaderTableOfContents headings={headings} onSelect={selectHeading} onClose={()=>setToc(false)}/>} {popover==="share"&&<ReaderSharePanel title={title} onCreateShare={onCreateShare} onClose={()=>setPopover(null)}/>} {popover==="save"&&<ReaderSavePanel onMarkdown={downloadMarkdown} onPdf={printPdf} onCopy={copyText} onClose={()=>setPopover(null)}/>} {guide&&<ReaderGuidebook onBack={closeGuide}/>} {notice&&<div className="reader-notice" role="status">{notice}</div>} {printing&&<article className="print-document"><MarkdownRenderer markdown={markdown}/></article>}
  </main>;
}

function Tool({icon,label,onClick,dataPopover}:{icon:string;label:string;onClick:()=>void;dataPopover?:string}){return <button data-reader-popover={dataPopover} onClick={onClick}><i className={`pixel-icon icon-${icon}`} aria-hidden="true"/><span>{label}</span></button>}
