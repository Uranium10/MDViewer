...
export default function NovelReader({documentId,title,markdown,preview=false,onBack,onShare}:Props){
 const{settings,setSettings,ready}=useReaderSettings();const[toc,setToc]=useState(false);const[drawer,setDrawer]=useState(false);const[progress,setProgress]=useState(0);const[target,setTarget]=useState<string|null>(null);const readerNode=useRef<HTMLElement>(null);const pull=useRef<{y:number;closing:boolean}|null>(null);const drawerDrag=useRef(0);const drawerNode=useRef<HTMLDivElement>(null);const drawerFrame=useRef<number|null>(null);const headings=useMemo(()=>extractHeadings(markdown),[markdown]);
 ...
 const updateProgress=useCallback((n:number)=>setProgress(old=>{const next=Math.max(0,Math.min(1,n));return Math.abs(old-next)<.0005?old:next}),[]);const toggleDrawer=useCallback(()=>setDrawer(v=>!v),[]);const widths={narrow:"640px",default:"720px",wide:"800px"};
