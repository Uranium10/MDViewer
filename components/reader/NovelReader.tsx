...
export default function NovelReader({documentId,title,markdown,preview=false,onBack,onShare}:Props){
 const{settings,setSettings,ready}=useReaderSettings();const[toc,setToc]=useState(false);const[drawer,setDrawer]=useState(false);const[guide,setGuide]=useState(false);const[progress,setProgress]=useState(0);const[target,setTarget]=useState<string|null>(null);const readerNode=useRef<HTMLElement>(null);const pull=useRef<{y:number;closing:boolean}|null>(null);const drawerDrag=useRef(0);const drawerNode=useRef<HTMLDivElement>(null);const drawerFrame=useRef<number|null>(null);const themeAudio=useRef<HTMLAudioElement|null>(null);const headings=useMemo(()=>extractHeadings(markdown),[markdown]);
 ...
 const openGuide=useCallback(()=>{if(guide)return;setDrawer(false);setToc(false);history.pushState({...history.state,mdBooksGuide:true},"",location.href);setGuide(true)},[guide]);
 ...
 return <main ref={readerNode} className={`novel-reader theme-${settings.theme} font-${settings.fontFamily} mode-${settings.mode} ${preview?"is-preview":""}`} style={{"--reader-font-size":`${settings.fontSize}px`,"--reader-line-height":settings.lineHeight,"--reader-max-width":widths[settings.width]} as React.CSSProperties} onPointerDown={pullDown} onPointerMove={pullMove} onPointerUp={pullEnd} onPointerCancel={pullEnd}>
  ...
  {toc&&<ReaderTableOfContents headings={headings} onSelect={selectHeading} onClose={()=>setToc(false)}/>} {guide&&<ReaderGuidebook onBack={closeGuide}/>}
 </main>
}
