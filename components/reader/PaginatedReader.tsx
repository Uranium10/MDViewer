...
export default function PaginatedReader({markdown,progress,bookmarks,onBookmarksChange,onProgress,onUiToggle,goToId,onPageChange}:Props){
  ...
  const down=(e:React.PointerEvent)=>{if(animating.current||(e.target as HTMLElement).closest("a,button,input,textarea,pre,[role=dialog]")||getSelection()?.toString())return;gesture.current={x:e.clientX,y:e.clientY,t:performance.now()};e.currentTarget.setPointerCapture(e.pointerId)};
