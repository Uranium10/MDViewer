...
export default function PaginatedReader({markdown,progress,onProgress,onUiToggle,goToId}:Props){
  ...
  return <div className="page-reader"><div className="page-paper" onPointerDown={down} onPointerMove={pointerMove} onPointerUp={finish} onPointerCancel={finish}><div className="page-viewport" ref={viewport}><div ref={content} className="page-columns"><MarkdownRenderer markdown={markdown}/></div></div></div><div className="page-navigation" onPointerDown={e=>e.stopPropagation()}>{editingPage?<form className="page-number-editor" onSubmit={submitPageEdit}><input ref={pageInput} type="number" inputMode="numeric" min={1} max={pages} value={pageDraft} onChange={e=>setPageDraft(e.target.value)} onBlur={commitPageEdit} onKeyDown={e=>{if(e.key==="Escape"){e.preventDefault();cancelEdit.current=true;setEditingPage(false)}}} aria-label={`이동할 페이지, 전체 ${pages}페이지`}/><span>/ {pages}</span></form>:<button className="page-number-button" onClick={beginPageEdit} aria-label={`현재 ${page+1}페이지, 전체 ${pages}페이지. 페이지 입력`}>{page+1} / {pages}</button>}<input className="page-scrubber" type="range" min={0} max={Math.max(0,pages-1)} step={1} value={page} onChange={e=>scrubTo(Number(e.target.value))} onPointerUp={finishScrub} onPointerCancel={finishScrub} onKeyUp={finishScrub} style={{"--page-progress":`${progressPercent}%`} as CSSProperties} aria-label={`페이지 빠른 탐색, 현재 ${page+1}페이지`}/></div></div>;
}
