...
type Props={markdown:string;progress:number;bookmarks:number[];onBookmarksChange:(next:number[])=>void;onProgress:(n:number)=>void;onUiToggle:()=>void;goToId:string|null};
...
export default function PaginatedReader({markdown,progress,bookmarks,onBookmarksChange,onProgress,onUiToggle,goToId}:Props){
  ...
  const scrubFrame=useRef<number|null>(null),pendingScrub=useRef<number|null>(null),pageInput=useRef<HTMLInputElement>(null),cancelEdit=useRef(false);
  const [pages,setPages]=useState(1),[page,setPage]=useState(0),[editingPage,setEditingPage]=useState(false),[pageDraft,setPageDraft]=useState("1");
  ...
  const scrubTo=(next:number)=>{pendingScrub.current=next;if(scrubFrame.current!==null)return;scrubFrame.current=requestAnimationFrame(()=>{scrubFrame.current=null;const pending=pendingScrub.current;pendingScrub.current=null;if(pending!==null)setPosition(pending)})};
  const finishScrub=()=>{if(scrubFrame.current!==null){cancelAnimationFrame(scrubFrame.current);scrubFrame.current=null}const pending=pendingScrub.current;pendingScrub.current=null;if(pending!==null)setPosition(pending)};
  ...
  return <div className="page-reader"><div className="page-paper" onPointerDown={down} onPointerMove={pointerMove} onPointerUp={finish} onPointerCancel={finish}><div className="page-viewport" ref={viewport}><div ref={content} className="page-columns"><MarkdownRenderer markdown={markdown}/></div></div></div><div className="page-navigation" onPointerDown={e=>e.stopPropagation()}><div className="page-indicator">{editingPage?<form className="page-number-editor" onSubmit={submitPageEdit}><input ref={pageInput} type="number" inputMode="numeric" min={1} max={pages} value={pageDraft} onChange={e=>setPageDraft(e.target.value)} onBlur={commitPageEdit} onKeyDown={e=>{if(e.key==="Escape"){e.preventDefault();cancelEdit.current=true;setEditingPage(false)}}} aria-label={`이동할 페이지, 전체 ${pages}페이지`}/><span>/ {pages}</span></form>:<button className="page-number-button" onClick={beginPageEdit} aria-label={`현재 ${page+1}페이지, 전체 ${pages}페이지. 페이지 입력`}>{page+1} / {pages}</button>}<button className={`bookmark-toggle ${isBookmarked?"is-active":""}`} onClick={toggleBookmark} aria-label={isBookmarked?"현재 페이지 북마크 해제":"현재 페이지 북마크"} aria-pressed={isBookmarked}><i aria-hidden="true"/></button></div><input className="page-scrubber" type="range" min={0} max={Math.max(0,pages-1)} step={1} value={page} onChange={e=>scrubTo(Number(e.target.value))} onPointerUp={finishScrub} onPointerCancel={finishScrub} onKeyUp={finishScrub} style={{"--page-progress":`${progressPercent}%`} as CSSProperties} aria-label={`페이지 빠른 탐색, 현재 ${page+1}페이지`}/></div></div>;
}
