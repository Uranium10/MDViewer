...
EPUB, Markdown과 텍스트 파일을 브라우저 안의 전자책처럼 읽고, 필요한 경우 텍스트 문서를 암호화된 링크로 공유하는 Next.js 웹 애플리케이션입니다.

## EPUB 읽기

- `.epub` 파일은 브라우저에서 EPUB.js로만 처리하며 서버에 업로드하지 않습니다.
- EPUB 내부의 스크립트 실행은 비활성화합니다.
- 목차, 페이지·스크롤 읽기, CFI 기반 이어읽기와 북마크, 글꼴·행간·테마 설정을 지원합니다.
- 최근 EPUB 원본과 읽기 위치는 브라우저의 IndexedDB와 로컬 저장소에 보관됩니다.
- DRM이 적용된 EPUB은 지원하지 않습니다.
